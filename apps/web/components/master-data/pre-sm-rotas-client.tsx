 ...
 */
export function PreSmRotasClient() {
  const t = useTranslations("PreSmRotas");
  ...
  const consulta = useQuery({
    queryKey: ["pre-sm-rotas"],
    queryFn: async () => {
      const res = await fetch("/api/admin/pre-sm-rotas");
  ...
  const carregar = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/admin/pre-sm-rotas", { method: "POST" });
      if (!res.ok) throw new Error(String(res.status));
    },
    onSuccess: () => {
  ...
  const definir = useMutation({
    ...
    onSuccess: (_d, v) => {
      void qc.invalidateQueries({ queryKey: ["pre-sm-rotas"] });
