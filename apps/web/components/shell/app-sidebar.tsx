...
export function AppSidebar({
  ...
  return (
    ...
    //
    // ── E NO TELEFONE ELE NÃO EXISTE (2026-08-28, a pedido) ───────────────────────────────────
    //
    // Esta barra tem 256px fixos. Num aparelho de 390px ela comia dois terços da tela e sobravam
    // cento e poucos pixels para o conteúdo — o painel do dia ficava ilegível não por culpa dele,
    // mas por falta de largura. Abaixo de `md` ela sai do fluxo inteiramente e a navegação passa a
    // ser a gaveta do `MenuMovel`, disparada pela barra de topo.
    //
    // `hidden md:flex` e não `w-0`: largura zero deixaria o elemento no fluxo, com borda, foco
    // alcançável por teclado e leitor de tela anunciando um menu invisível.
    <aside
      className={cn(
 ...
 *
 * O desktop a desenha dentro da barra fixa; o telefone, dentro da gaveta do `MenuMovel`. Extraí em
 * vez de duplicar porque duas cópias de uma navegação divergem em silêncio — alguém acrescenta uma
 * tela num lugar, e ela some no outro sem erro nenhum aparecer.
 *
 * `aoNavegar` existe só para a gaveta: clicar num item ali tem de FECHÁ-LA, senão a pessoa troca de
            ...
            {doGrupo.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  // Na gaveta do telefone, navegar tem de fechá-la. No desktop `aoNavegar` não
                  // existe e isto não faz nada.
                  onClick={aoNavegar}
