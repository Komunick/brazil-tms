...
(function () {
  ...
  const CONFIG = {
    ...
    //
    // Moram aqui, e nao em constantes no topo, pelo mesmo motivo de `tms` e
    // `token`: e o bloco CONFIG que o instalar.sh sabe preservar quando publica
    // codigo novo por cima. Vazias ou com COLE_, o heartbeat nao envia nada e o
    // robo roda exatamente como hoje.
    /** Ex.: https://status.braziltransports.com.br/api/push */
    hb_base: "COLE_A_BASE_DO_KUMA",
    /** Token do monitor "BOT · bsc-feed · vivo". */
    hb_vivo: "COLE_O_TOKEN_VIVO",
    /** Token do monitor "BOT · bsc-feed · ciclo horario". */
  ...
  function repetir() {
    ...
    const passo = async () => {
      ...
      try {
        await ciclo();
        HB.ciclo("leitura concluida", Date.now() - inicioDoCiclo);
      } catch (e) {
        erro("ciclo falhou:", e?.message ?? e);
        // Marca DOWN na hora, com o motivo, em vez de esperar os 4500 s de
        // tolerancia expirarem: o alerta chega mais de uma hora antes.
        HB.falhou(e?.message ?? String(e));
      }
      /**
