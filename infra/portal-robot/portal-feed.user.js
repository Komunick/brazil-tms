...
(function () {
  ...
  const CONFIG = {
    ...
    /** Token do monitor "BOT · portal-feed · vivo". */
    hb_vivo: "COLE_O_TOKEN_VIVO",
    /** Token do monitor "BOT · portal-feed · ciclo plano". */
  ...
  function repetir(nome, intervaloMs, tarefa, modo) {
    ...
    const passo = async () => {
      ...
      try {
        ...
        HB.ciclo(`${nome}: ${r.viagens} viagens`, Date.now() - t0);
      } catch (e) {
        erro(`${nome} falhou (tenta de novo no próximo ciclo):`, e?.message ?? e);
        HB.falhou(`${nome}: ${e?.message ?? e}`);
      } finally {
