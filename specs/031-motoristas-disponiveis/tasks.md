...
- [X] T001 Confirmar que a árvore está limpa e a branch é `feat/motoristas-disponiveis`, e que
...
- [X] T007 Criar `packages/db/src/fleet/motoristas-disponiveis.ts` com a consulta única (R5): varredura
...
- [X] T008 Em `motoristas-disponiveis.ts`, resolver o motorista por
...
- [X] T009 Em `motoristas-disponiveis.ts`, ordenar a escolha da última viagem nesta ordem:
      ...
      **(3)** no empate exato, a concluída ganha da cancelada; **(4)** `trip_id`, o desempate estável
      ...
      estação
- [X] T010 Em `motoristas-disponiveis.ts`, montar as placas com `placasDoPortal` de
...
- [X] T011 Em `motoristas-disponiveis.ts`, trazer o impedimento reusando `bloqueiosPorIdDoPortal` de
...
- [X] T012 Criar `packages/db/src/fleet/motoristas-disponiveis.test.ts` — **teste de fonte** que lê o
...
- [X] T013 Exportar `motoristasDisponiveis` no barril `packages/db/src/index.ts`

### A rota (etapa 3 do plano)

- [X] T014 Criar `apps/web/app/api/fleet/motoristas-disponiveis/route.ts` — `GET`, `requireAuth` +
      `requirePermission(ctx, "view_all_trips")`, **sem parâmetros** (contrato), devolvendo
      `{ motoristas, contagem: { disponiveis, aCaminho } }`
- [X] T015 Na rota, garantir que a lista vazia é **200** com `motoristas: []` e contagens em zero —
...
- [X] T016 [US1] Criar `apps/web/app/(shell)/motoristas-disponiveis/page.tsx` — guarda de servidor
      (`verifySession` + `can(user, "view_all_trips")`, redireciona se não), título e subtítulo
- [X] T017 [US1] Criar `apps/web/components/fleet/motoristas-disponiveis-client.tsx` com a consulta
...
- [X] T018 [P] [US1] Adicionar os rótulos em `apps/web/messages/pt-BR.json` — título, subtítulo,
      cabeçalhos das colunas, `FINALIZADO`, `CANCELADA`, as duas contagens e a frase de lista vazia
- [X] T019 [US1] Na tabela, as colunas na ordem da planilha: Nome · Origem · Destino · Início ·
      Conclusão · Cavalo · Carreta · Situação (FR-002)
- [X] T020 [US1] A coluna Carreta mostra **vazio assinalado** quando não há segunda placa — nunca a
      placa do cavalo (FR-003, armadilha 6)
- [X] T021 [US1] A coluna Situação usa `situacaoDaViagem` da etapa 1 e, para `a_caminho`, o rótulo de
      status que a Torre de Controle já tem em `pt-BR.json` — **não** criar um segundo catálogo (R9)
- [X] T022 [US1] Datas formatadas em São Paulo com o utilitário de formatação que já existe em
      `packages/shared` (`formatDateTime`) — não escrever formatação nova
- [X] T023 [US1] Ordem padrão: **disponível há mais tempo primeiro** (FR-006). Registrar no
...
- [X] T024 [US1] Estado vazio em palavras, nunca tabela vazia (FR-024)
...
- [X] T025 [US2] Distinguir visualmente `a_caminho` de `finalizado` na coluna Situação — cor e
...
- [X] T029 [US3] Selo de impedido na linha, ao lado do nome, com o motivo no `title` — a marca não
      pode ocupar a linha inteira nem empurrar as colunas (FR-017)
- [X] T030 [P] [US3] Rótulos dos dois motivos em `apps/web/messages/pt-BR.json` — bloqueado por nós e
      cadastro não ativo (FR-018)
- [X] T031 [US3] Confirmar que o motorista impedido **continua aparecendo** — esconder é a decisão
...
- [X] T032 Adicionar a entrada em `apps/web/lib/nav.ts` com `pai: "trips"`, `grupo: "operacao"`,
      `permission: "view_all_trips"` e um ícone — irmã de Minha Programação, como a spec decidiu
- [X] T033 [P] Rótulo do menu em `apps/web/messages/pt-BR.json`

---

## Phase 7: Polish & Cross-Cutting Concerns

- [X] T034 [P] Playwright em `apps/web/e2e/` — abrir a aba, conferir que as colunas existem, que a
      busca filtra sem recarregar e que o estado vazio aparece em palavras
- [X] T035 [P] Teste de invariante I1 (nenhuma escrita): varrer o módulo de leitura **e** a rota
...
- [X] T038 Rodar `pnpm lint` da **raiz** (`eslint .`), os quatro typechecks e `pnpm vitest run`
